...
for (const [name, query] of cases) {
  ...
  console.log(JSON.stringify({ name, ms, status: result.status, ranking: result.ranking, mode: result.explanationMode, ids: result.cards.map((c: { id: string }) => c.id) }));
  report.push({ name, ms, query, result });
}
